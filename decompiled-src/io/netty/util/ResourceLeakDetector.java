...
public class ResourceLeakDetector<T> {
    ...
    private final Set<String> reportedLeaks = Collections.newSetFromMap(new ConcurrentHashMap<K, V>());
    ...
    private DefaultResourceLeak track0(T obj, boolean force) {
        ...
        if (force || level == Level.PARANOID || level != Level.DISABLED && PlatformDependent.threadLocalRandom().nextInt(this.samplingInterval) == 0) {
            this.reportLeak();
            return new DefaultResourceLeak<T>(obj, this.refQueue, this.allLeaks, this.getInitialHint(this.resourceType));
        }
        return null;
    }
