...
extends AbstractReferenceCountedByteBuf {
    ...
    public ByteBuffer[] nioBuffers(int index, int length) {
        ...
        array = RecyclableArrayList.newInstance(this.buffers.length);
        c = this.findComponent(index);
        i = Component.access$200(c);
        adjustment = Component.access$000(c);
        s = c.buf;
        block7: while (true) {
...
lbl20:
                    ...
                    adjustment += s.readableBytes();
                    if ((length -= localLength) <= 0) {
                        var8_9 = array.toArray(EmptyArrays.EMPTY_BYTE_BUFFERS);
                        return var8_9;
                    }
                    s = this.buffer(++i);
                    continue block7;
