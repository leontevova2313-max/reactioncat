...
public final class HttpUtil {
    ...
    public static long normalizeAndGetContentLength(List<? extends CharSequence> contentLengthFields, boolean isHttp10OrEarlier, boolean allowDuplicateContentLengths) {
        ...
        firstField = contentLengthFields.get(0).toString();
        v0 = multipleContentLengths = contentLengthFields.size() > 1 || firstField.indexOf(44) >= 0;
        if (!multipleContentLengths || isHttp10OrEarlier) ** GOTO lbl13
        if (!allowDuplicateContentLengths) {
            throw new IllegalArgumentException("Multiple Content-Length values found: " + contentLengthFields);
        }
        firstValue = null;
        var6_8 = contentLengthFields.iterator();
        block2: while (true) {
            block13: {
                if (var6_8.hasNext()) break block13;
...
lbl13:
                ...
                try {
                    value = Long.parseLong(firstField);
            ...
            field = var6_8.next();
            var9_11 = tokens = field.toString().split(HttpUtil.COMMA_STRING, -1);
            var10_12 = var9_11.length;
            var11_13 = 0;
            while (true) {
                if (var11_13 >= var10_12) continue block2;
                token = var9_11[var11_13];
                trimmed = token.trim();
                if (firstValue == null) {
                    firstValue = trimmed;
