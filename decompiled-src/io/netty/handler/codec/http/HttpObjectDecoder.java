...
extends ByteToMessageDecoder {
    ...
    private static int skipWhiteSpaces(byte[] hex, int start, int length) {
        int i = 0;
        while (i < length) {
            if (!HttpObjectDecoder.isWhitespace(hex[start + i])) {
                return i;
            }
            ++i;
        }
    ...
    private static int findNonSPLenient(byte[] sb, int offset, int end) {
        ...
        while (true) {
            ...
            c = sb[result];
            if (!HttpObjectDecoder.isSPLenient(c)) break;
            ++result;
        }
    ...
    private static int findSPLenient(byte[] sb, int offset, int end) {
        int result = offset;
        while (result < end) {
            if (HttpObjectDecoder.isSPLenient(sb[result])) {
                return result;
            }
            ++result;
        }
    ...
    private static int findEndOfString(byte[] sb, int start, int end) {
        int result = end - 1;
        while (result > start) {
            if (!HttpObjectDecoder.isWhitespace(sb[result])) {
                return result + 1;
            }
            --result;
        }
