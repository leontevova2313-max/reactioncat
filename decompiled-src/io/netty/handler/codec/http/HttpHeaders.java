...
implements Iterable<Map.Entry<String, String>> {
    ...
    public boolean containsValue(CharSequence name, CharSequence value, boolean ignoreCase) {
        ...
        do {
            ...
            return false;
        } while (!HttpHeaders.containsCommaSeparatedTrimmed(itr.next(), value, ignoreCase));
        return true;
    }
        ...
        block8: {
            ...
            block9: {
                ...
                block7: {
                    block6: {
                        begin = 0;
                        if (!ignoreCase) break block6;
                        end2 = AsciiString.indexOf(rawNext, ',', begin);
                        if (end2 != -1) break block7;
                        if (AsciiString.contentEqualsIgnoreCase(AsciiString.trim(rawNext), expected)) {
                            return true;
                        }
                        break block8;
                    }
                    end = AsciiString.indexOf(rawNext, ',', begin);
                ...
                do {
                    if (!AsciiString.contentEqualsIgnoreCase(AsciiString.trim(rawNext.subSequence(begin, end2)), expected)) continue;
                    return true;
                } while ((end2 = AsciiString.indexOf(rawNext, ',', begin = end2 + 1)) != -1);
