...
final class PoolThreadCache {
    ...
    private static <T> MemoryRegionCache<T>[] createNormalCaches(int cacheSize, int maxCachedBufferCapacity, PoolArena<T> area) {
        if (cacheSize > 0 && maxCachedBufferCapacity > 0) {
            ...
            ArrayList<NormalMemoryRegionCache<T>> cache = new ArrayList<NormalMemoryRegionCache<T>>();
    ...
    private static void checkCacheMayLeak(MemoryRegionCache<?>[] caches, String type) {
        MemoryRegionCache<?>[] memoryRegionCacheArray = caches;
        int n = memoryRegionCacheArray.length;
        int n2 = 0;
        while (n2 < n) {
            MemoryRegionCache<?> cache = memoryRegionCacheArray[n2];
    ...
    private static int free(MemoryRegionCache<?>[] caches, boolean finalizer) {
        ...
        int n2 = 0;
        while (n2 < n) {
            MemoryRegionCache<?> c = memoryRegionCacheArray[n2];
