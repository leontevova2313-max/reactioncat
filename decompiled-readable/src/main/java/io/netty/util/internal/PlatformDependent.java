...
public final class PlatformDependent {
    ...
    public static <T> Queue<T> newSpscQueue() {
        return PlatformDependent.hasUnsafe() ? new SpscLinkedQueue<E>() : new SpscLinkedAtomicQueue<E>();
    }

    public static <T> Queue<T> newFixedMpscQueue(int capacity) {
        return PlatformDependent.hasUnsafe() ? new MpscArrayQueue<E>(capacity) : new MpscAtomicArrayQueue<E>(capacity);
    }
    ...
    private static void addClassifier(Set<String> allowed, Set<String> dest, String ... maybeClassifiers) {
        String[] stringArray = maybeClassifiers;
        int n = stringArray.length;
        int n2 = 0;
        while (n2 < n) {
    ...
    private static final class Mpsc {
        ...
        static <T> Queue<T> newChunkedMpscQueue(int chunkSize, int capacity) {
            return USE_MPSC_CHUNKED_ARRAY_QUEUE ? new MpscChunkedArrayQueue<E>(chunkSize, capacity) : new MpscChunkedAtomicArrayQueue<E>(chunkSize, capacity);
        }

        static <T> Queue<T> newMpscQueue() {
            return USE_MPSC_CHUNKED_ARRAY_QUEUE ? new MpscUnboundedArrayQueue<E>(1024) : new MpscUnboundedAtomicArrayQueue<E>(1024);
        }
