    ...
    implements Parser {
        ...
        public HostsFileEntriesProvider parse(Reader reader) throws IOException {
            block16: {
                ...
                ipv4Entries = new HashMap<String, List<InetAddress>>();
                ipv6Entries = new HashMap<String, List<InetAddress>>();
lbl7:
                // 2 sources

                while (true) {
                    if ((line = buff.readLine()) != null) {
                        commentPosition = line.indexOf(35);
                        if (commentPosition != -1) {
                            line = line.substring(0, commentPosition);
                        }
                        if ((line = line.trim()).isEmpty()) continue;
                        lineParts = new ArrayList<String>();
                        for (String s : ParserImpl.WHITESPACES.split(line)) {
                    ...
                    var6_7 = ipv4Entries.isEmpty() != false && ipv6Entries.isEmpty() != false ? HostsFileEntriesProvider.EMPTY : new HostsFileEntriesProvider(ipv4Entries, ipv6Entries);
                    return var6_7;
                }
                finally {
                    try {
            ...
            i = 1;
            while (true) {
                if (i < lineParts.size()) ** break;
                ** continue;
                hostname = (String)lineParts.get(i);
                hostnameLower = hostname.toLowerCase(Locale.ENGLISH);
                address = InetAddress.getByAddress(hostname, ipBytes);
