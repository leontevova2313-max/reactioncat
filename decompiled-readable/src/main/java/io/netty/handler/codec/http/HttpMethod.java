...
implements Comparable<HttpMethod> {
    ...
    private static final EnumNameMap<HttpMethod> methodMap = new EnumNameMap<T>(new EnumNameMap.Node<HttpMethod>(OPTIONS.toString(), OPTIONS), new EnumNameMap.Node<HttpMethod>(GET.toString(), GET), new EnumNameMap.Node<HttpMethod>(HEAD.toString(), HEAD), new EnumNameMap.Node<HttpMethod>(POST.toString(), POST), new EnumNameMap.Node<HttpMethod>(PUT.toString(), PUT), new EnumNameMap.Node<HttpMethod>(PATCH.toString(), PATCH), new EnumNameMap.Node<HttpMethod>(DELETE.toString(), DELETE), new EnumNameMap.Node<HttpMethod>(TRACE.toString(), TRACE), new EnumNameMap.Node<HttpMethod>(CONNECT.toString(), CONNECT));
    ...
    private static final class EnumNameMap<T> {
        ...
        EnumNameMap(Node<T> ... nodes) {
            ...
            Node<T>[] nodeArray = nodes;
            int n = nodeArray.length;
            int n2 = 0;
            while (n2 < n) {
                Node<T> node = nodeArray[n2];
